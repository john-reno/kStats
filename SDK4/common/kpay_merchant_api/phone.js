...
export default class KpayMerchantApi {
  ...
  getHistory() {
    return this._apiData.get(common.HISTORY);
  }
  ...
  fetchHistory() {
    return this._fetchData(common.HISTORY);
  }
...
prv_fetch_functions[common.HISTORY] = prv_fetchRemoteHistory;
...
function prv_fetchRemoteSummary(apiKey, success, error) {
  ...
  var url = 'https://api.kiezelpay.com/api/merchant/summary?key=' + apiKey;
...
function prv_fetchRemoteToday(apiKey, success, error) {
  ...
  var url = 'https://api.kiezelpay.com/api/merchant/today?key=' + apiKey + '&offset=' + (new Date()).getTimezoneOffset();
...
function prv_fetchRemoteYesterday(apiKey, success, error) {
  ...
  var url = 'https://api.kiezelpay.com/api/merchant/yesterday?key=' + apiKey + '&offset=' + (new Date()).getTimezoneOffset();
  prv_fetchAndSend(common.YESTERDAY, url, success, error);
}

function prv_fetchRemoteHistory(apiKey, success, error) {
  console.log('Fetching KPay Merchant API \'history\' data from KPay servers...');
  var url = 'https://api.kiezelpay.com/api/merchant/history?key=' + apiKey + '&amount=7' + '&sort=desc';
  prv_fetchAndSend(common.HISTORY, url, success, error);
}
